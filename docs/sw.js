...
self.addEventListener('fetch', e => {
  ...
  // shell: network first with HTTP-cache revalidation (GitHub Pages sets max-age=600), fall back to cache when offline
  e.respondWith(fetch(e.request, { cache: 'no-cache' }).then(r => { const copy = r.clone(); caches.open(SHELL).then(c => c.put(e.request, copy)); return r; }).catch(() => caches.match(e.request, { ignoreSearch: true }).then(r => r || caches.match('./index.html'))));
});
